...
import org.jeecg.modules.restfulupload.util.*;
...
@Slf4j
@RestController
@RequestMapping("/restful/down")
public class RestfulDownController {
    @Autowired
     ...
     * 多文件打包下载
     */
    @RequestMapping(value = "/zipMultiFile", method = RequestMethod.POST)
    public Result<ZipMultiFileModel> zipMultiFile(@RequestBody ZipMultiFileModel zipMultiFileModel) {
        Result<ZipMultiFileModel> result = new Result<ZipMultiFileModel>();

        String  returnInformation= ZipMultiFileModelUntil.zipMultiFileModelUntitl(zipMultiFileModel);//校验必填字段
        result.setResult(zipMultiFileModel);
        if(returnInformation.equals("trues")){
            String file=zipMultiFileModel.getFileId();
            String[] strs = file.split("/");

            File[] srcFiles =  new File[strs.length];
            File zipFile = new File("E:\\镇雄\\zip\\202005\\11\\title.zip");
            for (int i = 0, len=strs.length; i<len; i++) {
                srcFiles[i]= new File (strs[i]);
            }
            try {
                ZipMultiFile.zipFiles(srcFiles,zipFile);//生成file文档
